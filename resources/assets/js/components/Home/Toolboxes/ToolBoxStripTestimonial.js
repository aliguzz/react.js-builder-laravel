...
const addImagesToPage = (event) => {
  ...
  var elementClickedHtml = window.stripAbout[index].html;
...
export default class ToolBoxStripTestimonial extends React.Component {
  render() {
    ...
    window.stripTestimonial = container.boxes;
    return (
      <div style={divStyle}>
        {
          _.map(container.boxes, function (item, index) {
            ...
            return (<div className="singleElementDragger" onDragStart={(event) => {
              var item = event.target;
              var index = item.getAttribute('data-index');
              var text = window.stripTestimonial[index].html;
