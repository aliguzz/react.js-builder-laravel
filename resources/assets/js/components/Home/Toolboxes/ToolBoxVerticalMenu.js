...
const addImagesToPage = (event) => {
  ...
  var elementClickedHtml = window.menuverticalarray[index].html;
...
export default class ToolBoxVerticalMenu extends React.Component {
  render() {
    ...
    window.menuverticalarray = container.boxes;
    return (
      <div style={divStyle}>
        {
          _.map(container.boxes, function (item, index) {
            ...
            return (<div className="singleElementDragger" onDragStart={(event) => {
              var item = event.target;
              var index = item.getAttribute('data-index');
              var text = window.menuverticalarray[index].html;
