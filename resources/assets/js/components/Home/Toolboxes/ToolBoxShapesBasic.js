...
const addImagesToPage = (event) => {
  ...
  var elementClickedHtml = window.shapesbasicarray[index].html;
...
export default class ToolBoxShapesBasic extends React.Component {
  render() {
    ...
    window.shapesbasicarray = container.boxes;
    return (
      <div style={divStyle}>
        {
          _.map(container.boxes, function (item, index) {
            ...
            return (<div className="singleElementDragger" onDragStart={(event) => {
              var item = event.target;
              var index = item.getAttribute('data-index');
              var text = window.shapesbasicarray[index].html;
