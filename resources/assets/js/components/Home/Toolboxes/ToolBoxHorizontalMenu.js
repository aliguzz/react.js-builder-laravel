...
const addImagesToPage = (event) => {
  ...
  var elementClickedHtml = window.menuhorizontalarray[index].html;
...
export default class ToolBoxHorizontalMenu extends React.Component {
  render() {
    ...
    window.menuhorizontalarray = container.boxes;
    return (
      <div style={divStyle}>
        {
          _.map(container.boxes, function (item, index) {
            ...
            return (<div className="singleElementDragger" onDragStart={(event) => {
              var item = event.target;
              var index = item.getAttribute('data-index');
              var text = window.menuhorizontalarray[index].html;
