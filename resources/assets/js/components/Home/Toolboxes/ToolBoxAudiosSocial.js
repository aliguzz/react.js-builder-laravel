...
const addImagesToPage = (event) => {
  ...
  var elementClickedHtml = window.audiosocialarray[index].html;
...
export default class ToolBoxAudiosSocial extends React.Component {
  render() {
    ...
    window.audiosocialarray = container.boxes;
    return (
      <div style={divStyle}>
        {
          _.map(container.boxes, function (item, index) {
            ...
            return (<div className="singleElementDragger" onDragStart={(event) => {
              var item = event.target;
              var index = item.getAttribute('data-index');
              var text = window.audiosocialarray[index].html;
