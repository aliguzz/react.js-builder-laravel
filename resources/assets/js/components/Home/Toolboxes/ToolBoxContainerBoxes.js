...
const addImagesToPage = (event) => {
  ...
  var elementClickedHtml = window.menuthemedarray[index].html;
...
export default class ToolBoxContainerBoxes extends React.Component {
  render() {
    ...
    window.toolBoxContainerBoxesArray = container.boxes;
    return (
      <div style={divStyle}>
        {
          _.map(container.boxes, function (item, index) {
            ...
            return (<div className="singleElementDragger" onDragStart={(event) => {
              var item = event.target;
              var index = item.getAttribute('data-index');
              var text = window.toolBoxContainerBoxesArray[index].html;
