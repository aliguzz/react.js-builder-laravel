...
const addImagesToPage = (event) => {
  ...
  var elementClickedHtml = window.shapeshorizontalarray[index].html;
...
export default class ToolBoxShapesHorizontal extends React.Component {
  render() {
    ...
    window.shapeshorizontalarray = container.boxes;
    return (
      <div style={divStyle}>
        {
          _.map(container.boxes, function (item, index) {
            ...
            return (<div className="singleElementDragger" onDragStart={(event) => {
              var item = event.target;
              var index = item.getAttribute('data-index');
              var text = window.shapeshorizontalarray[index].html;
