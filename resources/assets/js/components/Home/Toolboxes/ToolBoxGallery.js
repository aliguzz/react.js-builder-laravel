...
const addImagesToPage = (event) => {
  ...
  var elementClickedHtml = window.gallaryarray[index].html;
...
export default class ToolBoxGallery extends React.Component {
  render() {
    ...
    window.gallaryarray = container.boxes;
    return (
      <div style={divStyle}>
        {
          _.map(container.boxes, function (item, index) {
            ...
            return (<div className="singleElementDragger" onDragStart={(event) => {
              var item = event.target;
              var index = item.getAttribute('data-index');
              var text = window.gallaryarray[index].html;
