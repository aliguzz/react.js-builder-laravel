...
const addImagesToPage = (event) => {
  ...
  var elementClickedHtml = window.shapesarrowsarray[index].html;
...
export default class ToolBoxShapesArrows extends React.Component {
  render() {
    ...
    window.shapesarrowsarray = container.boxes;
    return (
      <div style={divStyle}>
        {
          _.map(container.boxes, function (item, index) {
            ...
            return (<div className="singleElementDragger" onDragStart={(event) => {
              var item = event.target;
              var index = item.getAttribute('data-index');
              var text = window.shapesarrowsarray[index].html;
