...
const addImagesToPage = (event) => {
  ...
  var elementClickedHtml = window.videosocialarray[index].html;
...
export default class ToolBoxVideosSocial extends React.Component {
  render() {
    ...
    window.videosocialarray = container.boxes;
    return (
      <div style={divStyle}>
        {
          _.map(container.boxes, function (item, index) {
            ...
            return (<div className="singleElementDragger" onDragStart={(event) => {
              var item = event.target;
              var index = item.getAttribute('data-index');
              var text = window.videosocialarray[index].html;
