...
const addImagesToPage = (event) => {
  ...
  var elementClickedHtml = window.menuanchorarray[index].html;
...
export default class ToolBoxAnchorMenu extends React.Component {
  render() {
    ...
    window.menuanchorarray = container.boxes;
    return (
      <div style={divStyle}>
        {
          _.map(container.boxes, function (item, index) {
            ...
            return (<div className="singleElementDragger" onDragStart={(event) => {
              var item = event.target;
              var index = item.getAttribute('data-index');
              var text = window.menuanchorarray[index].html;
